...
from stl_pi_planner.STL.predicate import DynamicLinearPredicate
...
def inside_rectangle_formula(bounds, y1_index, y2_index, d, name=None):
    ...
    # Take the conjunction across all the sides
...
def dynamic_inside_rectangle_formula(centers, width, height, y1_index, y2_index, d, name=None):
    """
    ...
    a1 = np.zeros((d, len(centers)))
    a1[y1_index, :] = 1.0
    b1 = np.array([center[0] - 0.5 * width for center in centers])
    right = DynamicLinearPredicate(a1, b1)

    b2 = np.array([center[0] + 0.5 * width for center in centers])
    left = DynamicLinearPredicate(-a1, -b2)

    a3 = np.zeros((d, len(centers)))
    a3[y2_index, :] = 1.0
    b3 = np.array([center[1] - 0.5 * height for center in centers])
    top = DynamicLinearPredicate(a3, b3)

    b4 = np.array([center[1] + 0.5 * height for center in centers])
    bottom = DynamicLinearPredicate(-a3, -b4)

    # Take the disjunction across all the sides
...
def dynamic_outside_rectangle_formula(centers, width, height, y1_index, y2_index, d, name=None):
    """
    Create an STL formula representing being outside a
    rectangle with the given bounds:
