"""
Trace: run ONE episode capturing per-iteration trajectory for our Adaptive-PI.
...
def trace_task(task_id, out_dir):
    ...
    sol = p.solutions['PI']
    traj_per_iter = []
    if p.pi_record_y_opt is not None:
    ...
    path = os.path.join(out_dir, f'trace_ours_task{task_id}.json')
    with open(path,'w') as f: json.dump(result, f)
    print(f'[trace] Adaptive-PI Task {task_id}: {len(traj_per_iter)} iters, rho={sol.rho:.3f} → {path}')
